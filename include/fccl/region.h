...
namespace fccl {
	...
	class region {
		public:
			...
			template <typename RNG> region(RNG rng) : gen(rng) { };
			...
			point next(std::vector<point>::iterator first,
			...
			double part(const point &p0, const point &pp,
				    const std::vector<rate> &rr) const {
				if (rr.size() != pp.size())
					throw std::invalid_argument("rates not compatible with this point");
